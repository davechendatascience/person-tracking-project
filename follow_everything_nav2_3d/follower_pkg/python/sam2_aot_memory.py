...
# This file lives in two layouts: the host checkout
#   <project_root>/follow_everything_nav2_3d/follower_pkg/python/sam2_aot_memory.py
# and the sim container, where follow_everything_nav2_3d is bind-mounted at /ws
# and EdgeTAM at /opt/EdgeTAM (see docker-compose.yml). So parents[2] is
# follow_everything_nav2_3d (or /ws), NOT the project root — resolve EdgeTAM and
# the SAM2 checkpoints against several candidate roots (env override wins).
_THIS_DIR = Path(__file__).resolve().parent
_PROJECT_ROOT = _THIS_DIR.parents[2]   # follow_everything_nav2_3d  (or /ws)


def _resolve_edgetam_dir() -> Path:
    """EdgeTAM checkout dir, honouring EDGETAM_DIR, then host/sim layouts."""
    cands = [os.environ.get("EDGETAM_DIR"),
             _PROJECT_ROOT / "EdgeTAM",         # sibling checkout
             _PROJECT_ROOT.parent / "EdgeTAM",  # <project_root>/EdgeTAM (host)
             Path("/opt/EdgeTAM")]              # docker-compose mount (sim)
    for c in cands:
        if c and Path(c).is_dir():
            return Path(c)
    return _PROJECT_ROOT / "EdgeTAM"


def _resolve_ckpt(name: str) -> Path:
    """Locate a SAM2 checkpoint by filename across the candidate roots."""
    for root in (_PROJECT_ROOT, _PROJECT_ROOT.parent, Path("/opt")):
        p = root / name
        if p.exists():
            return p
    return _PROJECT_ROOT.parent / name


_EDGETAM_DIR = _resolve_edgetam_dir()
...
class SAM2AOTMemoryTracker:
    ...
    _BACKBONES = {
        "sam2-large": ("configs/sam2.1/sam2.1_hiera_l.yaml",
                       _resolve_ckpt("sam2.1_hiera_large.pt")),
        "sam2-small": ("configs/sam2.1/sam2.1_hiera_s.yaml",
                       _resolve_ckpt("sam2.1_hiera_small.pt")),
        "sam2-tiny": ("configs/sam2.1/sam2.1_hiera_t.yaml",
                      _resolve_ckpt("sam2.1_hiera_tiny.pt")),
    ...
    # ------------------------------------------------------------------
    # Per-run lifecycle + single-frame steps. These are shared by both the
    # batch generator (track_sequence, used by run_video) and the streaming
    # adapter (SAM2AOTMemoryStreamingTracker, used by the nav2 ROS worker),
    # so the two paths run byte-identical memory management.
    # ------------------------------------------------------------------
    def _begin(self) -> None:
        """Build the predictor (once) and reset all per-run memory state."""
        self._build()
    ...
    def _make_amp(self):
        """BF16/FP16 autocast over the per-frame pipeline (encoder + memory
        attention + decoder) — ~2.5x on this HW. nullcontext in fp32/CPU."""
        import contextlib
        import torch
    ...
    def _maybe_empty_cache(self, frame_idx: int) -> None:
        import torch
    ...
    def _seed(self, rgb: np.ndarray,
              initial_bbox: Optional[np.ndarray] = None,
              initial_mask: Optional[np.ndarray] = None):
        """Frame 0: build inference_state and prompt the tracker with a box
        (preferred — SAM2 locks more reliably on a dense box) or mask. Records
        the seed identity anchor. Caller must hold inference_mode + autocast.
        Returns (state, TrackResult, mask_px)."""
    ...
    def _step(self, state, frame_idx: int, rgb: np.ndarray) -> TrackResult:
        """Advance one frame: propagate (SAM2 writes memory natively), then run
        the full AOT long/short-term memory management — appearance-gated LT
        promotion, live distractor rejection, periodic self-consistency audit,
        bounded short-term eviction. Caller must hold inference_mode + autocast.
        Returns the frame's TrackResult (empty when a distractor is suppressed).
        """
        h, w = rgb.shape[:2]
        predictor = self._predictor
        ...
        if distractor:
            ...
            if self._pending_fail:
                ...
                self._pending_fail.clear()
            res = TrackResult(None, 0.0, None, False)
    ...
    def track_sequence(self,
        ...
        import torch
        self._begin()
        with torch.inference_mode(), self._make_amp():
            state = None
            for frame_idx, rgb in enumerate(frames):
                if frame_idx == 0:
                    state, res, _ = self._seed(rgb, initial_bbox, initial_mask)
                    yield 0, res
                    continue
                res = self._step(state, frame_idx, rgb)
                yield frame_idx, res
                self._maybe_empty_cache(frame_idx)


class SAM2AOTMemoryStreamingTracker:
    """Frame-by-frame ``.initialize()`` / ``.track()`` adapter over
    :class:`SAM2AOTMemoryTracker`, matching the streaming API the nav2 ROS
    worker (follow_everything_nav2_3d/follower_pkg/python/edgetam_tracker.py)
    drives:

      • ``initialize(pil, init_mask=None, bbox=[x, y, w, h]) -> {"pred_mask": u8}``
      • ``track(pil, init=False)                            -> {"pred_mask": u8}``
      • ``.inference_state``  — read by the node's VRAM-eviction helper.

    All memory management (appearance-gated LT promotion, distractor rejection,
    self-consistency audit, bounded short-term eviction) is the real thing from
    ``SAM2AOTMemoryTracker`` — this replaces the node's old size-only inline
    ``_mem_promote`` copy. ``bbox`` is xywh (as the worker emits); we convert to
    the core's xyxy. ``pred_mask`` is HxW uint8 (0/1), the format the node's
    ``_make_result`` / overlay expect.
    """

    def __init__(self, **kwargs):
        self.core = SAM2AOTMemoryTracker(**kwargs)
        self.core._build()          # pay the (slow) build now, before frame 0
        self._state = None
        self._frame_idx = 0

    # The node's _evict_edgetam_state(tracker, ...) reads this to prune old
    # per-frame state; expose the core's live inference_state dict.
    @property
    ...
    def _to_rgb(image) -> np.ndarray:
        """Accept a PIL.Image or ndarray; return a contiguous HxWx3 uint8 RGB."""
        return np.ascontiguousarray(np.asarray(image))

    @staticmethod
    def _mask_u8(res: TrackResult, hw: Tuple[int, int]) -> np.ndarray:
        h, w = hw
        if res.mask is None:
            return np.zeros((h, w), dtype=np.uint8)
        return res.mask.astype(np.uint8)

    def initialize(self, image, init_mask=None, bbox=None):
        """Seed on frame 0. Prefers bbox (xywh); init_mask is the fallback."""
        import torch
        rgb = self._to_rgb(image)
        ibbox = None
        if bbox is not None:
            x, y, w, h = [float(v) for v in bbox]   # xywh from the worker
            ibbox = np.array([x, y, x + w, y + h], dtype=np.float32)
        imask = None if init_mask is None else (np.asarray(init_mask) > 0)
        self.core._begin()
        with torch.inference_mode(), self.core._make_amp():
            self._state, res, _ = self.core._seed(rgb, ibbox, imask)
        self._frame_idx = 0
        return {"pred_mask": self._mask_u8(res, rgb.shape[:2])}

    def track(self, image, init=False):
        """Advance one frame and run the full memory management for it."""
        import torch
        rgb = self._to_rgb(image)
        if not init:
            self._frame_idx += 1
        with torch.inference_mode(), self.core._make_amp():
            res = self.core._step(self._state, self._frame_idx, rgb)
        return {"pred_mask": self._mask_u8(res, rgb.shape[:2])}
